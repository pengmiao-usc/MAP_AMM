...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100,num_channel=3):
        ...
        self.conv3_x = self._make_layer(block, DIM*2, num_block[1], 2)#strid=2 makes the output dim half
...
class ResNet_tiny(nn.Module):
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride):
        strides = [stride] + [1] * (num_blocks - 1) #first block "stride", other blocks stride=2=1
        layers = []
        for stride in strides:
            layers.append(block(self.in_channels, out_channels, stride))
            self.in_channels = out_channels * block.expansion
    ...
    def forward(self, x):
        output = self.conv1(x) #torch.Size([3025, 4, 10, 10])
        output = self.conv2_x(output) #torch.Size([3025, 4, 10, 10])
        output = self.conv3_x(output) #torch.Size([3025, 8, 5, 5])
        output = self.conv4_x(output) #torch.Size([3025, 16, 3, 3])
        output = self.avg_pool(output) #torch.Size([3025, 16, 1, 1])
        output = output.view(output.size(0), -1) #torch.Size([3025, 16])
        output = self.fc(output)#torch.Size([3025, 256])
