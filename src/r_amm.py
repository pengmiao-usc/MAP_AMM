...
def im2col_conv(input_data, kernel_weights, kernel_size, padding=1, stride=1):
    col_matrix, reshaped_kernel_weights, output_shape = im2col_transform(input_data, kernel_weights,
                                                                         kernel_weights.shape[-1], padding, stride)
    conv_result = np.dot(col_matrix, reshaped_kernel_weights.transpose())
    output = conv_result.reshape(*output_shape)
    return output


##

def manual_batch_norm_2d(input, weight, bias, mean, var, eps=1e-5, affine=True):  # model must be eval() state
    input = torch.tensor(input)
    mean = torch.tensor(mean)
    var = torch.tensor(var)
    input = (input - mean[None, :, None, None]) / (torch.sqrt(var[None, :, None, None] + eps))
    if affine:
...
def im2col_conv_bn_fold_amm(input_data, kernel_weights, bn_weight, bn_bias, mean, var, eps=1e-5,
    ...
    if est is None:
        est = PQ_AMM_CNN(ncodebooks, ncentroids)
...
class ResNet_Tiny_Manual():
    ...
    def resnet_mlp(self, input_data, weights, bias, act_fn=None, ncodebooks=1, ncentroids=64, est=None):
    ...
    def train_amm(self, input_data):
        layer1 = self.resnet_im2col_conv_bn_fold_amm(input_data, *self.layer1_weights, *self.layer1_bn_buffers,
                                              ncodebooks=self.n_subspace[0], ncentroids=self.k_cluster[0])
        layer1 = self.relu(0, layer1)

        layer2 = self.train_block_amm(layer1, self.layer2_weights, self.layer2_bn_buffers, stride=1,
                                      n=self.n_subspace[1], k=self.k_cluster[1])
        layer3 = self.train_block_amm(layer2, self.layer3_weights, self.layer3_bn_buffers, stride=2,
                                      n=self.n_subspace[2], k=self.k_cluster[2])
        layer4 = self.train_block_amm(layer3, self.layer4_weights, self.layer4_bn_buffers, stride=2,
                                      n=self.n_subspace[3], k=self.k_cluster[3])

        output = self.avg_pool(torch.tensor(layer4)).view(input_data.shape[0], -1).detach().numpy()

        output = self.resnet_mlp(output, self.layer_fc_weights[0].transpose(), self.layer_fc_weights[1],
                              act_fn=None, ncodebooks=self.n_subspace[4], ncentroids=self.k_cluster[4])
        output = self.sigmoid(torch.tensor(output)).detach().numpy()
    ...
    def eval_amm(self, input_data):
        ...
        layer2 = self.eval_block_amm(layer1, self.layer2_weights, self.layer2_bn_buffers, stride=1,
                                     n=self.n_subspace[1], k=self.k_cluster[1])
        layer3 = self.eval_block_amm(layer2, self.layer3_weights, self.layer3_bn_buffers, stride=2,
                                     n=self.n_subspace[2], k=self.k_cluster[2])
        layer4 = self.eval_block_amm(layer3, self.layer4_weights, self.layer4_bn_buffers, stride=2,
                                     n=self.n_subspace[3], k=self.k_cluster[3])
